...
| `colcon build` 시 `smartfarm_interfaces`에서 `ModuleNotFoundError: No module named 'em'` / `'catkin_pkg'` | `./scripts/fix_ros_build_env.sh` 실행 — `$ROS_DISTRO`를 감지해서 `.venv`에 `catkin_pkg`/`lark`/맞는 버전의 `empy`를 자동으로 설치한다(Humble/Iron=3.3.4 고정, Jazzy/Kilted/Rolling=4.x). ROS2 배포판이 여러 개 설치된 경우 사용할 `/opt/ros/<distro>/setup.bash`를 먼저 source한다. 배포판별 버전을 섞어 쓰면 다른 에러로 빌드가 깨지므로 수동으로 `pip install empy`만 하지 말 것 |
