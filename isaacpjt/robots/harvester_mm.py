...
# m0617 MoveIt 홈 자세 — 측면에서 어깨→팔꿈치→손목이 '>'로 접히고 TCP가 왼쪽을
# 향하는 주행/관찰 자세. joint_1=180°로 이 접힘 평면 전체를 라이다 전방으로 돌린다.
# 관절 1 회전 전 팔 로컬 측면 FK(arm mount 제외):
#   shoulder=(0.000, 0.153), elbow=(0.732, 0.575),
#   wrist=(0.096, 0.943), harvest_tcp=(-0.145, 0.943)  [x,z] m
# Isaac·SRDF·ros2_control initial_value·mm_motion_bridge HOME_Q와 반드시 같아야 한다.
# 근위→원위 순서 유지(_preset_joint가 원위 링크를 통째로 돌리므로 순서가 중요).
HOME_POSE_DEG = (("joint_1", 180.0),
                 ("joint_2", 60.0),
                 ("joint_3", -120.0),
                 ("joint_4", 0.0),
                 ("joint_5", -30.0),
...
class HarvestMM(_ScoopMM):
    ...
    def _preset_pose(self, stage: Usd.Stage, arm_path: str, log) -> None:
        """m0617 홈은 관절 상태로만 설정하고 링크 Xform에는 굽지 않는다.

        부모의 UR10e용 구현은 정지 뷰포트에서도 홈을 보이게 하려고 원위 링크
        Xform을 직접 회전한 뒤 같은 관절각을 JointState에도 기록한다. m0617은
        그 상태를 다시 ros2_control/PhysX 관절각으로 적용하면 Isaac 형상에 홈
        회전이 이중 반영되어, 같은 joint_states를 받은 MoveIt URDF FK와 달라진다.

        mm.MMDriver.configure()가 articulation default state를 같은 HOME_POSE로
        설정하므로 여기서는 PhysX 초기 관절값과 drive target만 기록한다.
        """
        for joint_name, degrees in self.HOME_POSE:
            joint = stage.GetPrimAtPath(f"{arm_path}/joints/{joint_name}")
            if not joint.IsValid():
                log(
                    f"[Harvester] ⚠ {joint_name} 없음 — "
                    "m0617 홈 관절 초기값 스킵")
                continue
            state = PhysxSchema.JointStateAPI.Apply(joint, "angular")
            state.CreatePositionAttr().Set(float(degrees))
            state.CreateVelocityAttr().Set(0.0)
            UsdPhysics.DriveAPI(
                joint, "angular").CreateTargetPositionAttr().Set(float(degrees))
            log(
                f"[Harvester] 시작자세 관절값: {joint_name} "
                f"{degrees:+.0f}° — 링크 Xform 사전회전 없음")
