...
GOAL_POS_BY_COLOR_NAME = {
    "blue": BLUE_GOAL_POS,
    "green": GREEN_GOAL_POS,
}
...
def initialize_robot(robot, world):
    """로봇/그리퍼를 초기화하되 USD에 저장된 로봇 자세는 유지한다."""
    robot.initialize()
    ...
    # 모든 관절을 0으로 덮어쓰면 M0609가 부자연스럽게 뻗은 자세가 된다.
    # World.reset()이 복원한 USD 기본 자세에서 RMPFlow 제어를 시작한다.
...
def main():
    ...
    # ── D-3. ROS2 /color_id 구독자 (선택 사항) ───────────────
    rclpy.init()
    color_sub = ColorIdSubscriber()
    print(f"  [OK] '{COLOR_ID_TOPIC}' 구독 시작 (미연결이어도 자체 색상으로 계속 진행)")

    # ── D-4. 한 라운드 = 파랑 큐브 + 초록 큐브를 둘 다 옮겨야 끝남 ──
    # SELECT      : 아직 옮기지 않은 큐브 중 하나를 랜덤 선택해 pick 영역 위(DROP_HEIGHT)에서
    #               낙하시키고 색상에 맞는 기본 place 목표를 즉시 지정함
    # DROP_SETTLE : 낙하한 큐브가 테이블에 착지해 멈출 때까지 대기
    # PICK_PLACE  : ROS 색상값이 오면 place 시작 전까지 그 값을 우선 반영하고,
    #               값이 없어도 선택된 큐브의 실제 색상 목표로 끝까지 수행함
    # CLEANUP     : 아직 안 옮긴 큐브가 남아있으면 SELECT로 돌아가 이어서 진행
    ...
    while simulation_app.is_running():
        ...
        if is_playing:
            if trial_phase == "SELECT":
                round_num += 1
                is_last_cube = len(remaining_colors) == 1  # 이번이 마지막 남은 큐브인지 확인
                ...
                goal_pos = GOAL_POS_BY_COLOR_NAME[active_color].copy()
                print(f"  [목표] {active_color} 큐브 → {goal_pos} (자체 색상 기본값)")

                # 첫 번째와 두 번째 큐브 모두 pick 영역 위에서 낙하시킨 뒤 집는다.
                # 첫 번째 처리 때만 다른 큐브를 공중 대기 위치에 유지한다.
                # 두 번째 처리 때 inactive_color는 이미 마커에 배치된 큐브이므로
                # 위치를 다시 바꾸지 않는다.
                pick_xy = random_pick_xy()
                task.drop_cube(active_color, pick_xy)
                if not is_last_cube:
                    task.float_cube(inactive_color)
                ...
                # place 이동(event 5) 전까지 유효한 ROS 색상값이 오면 자체
                # 색상 기본값을 갱신한다. 발행자가 없어도 작업은 멈추지 않는다.
                color_id = color_sub.pop_latest()
                if color_id in GOAL_POS_BY_COLOR_ID and controller.get_current_event() < 5:
                    goal_pos = GOAL_POS_BY_COLOR_ID[color_id].copy()
                    print(f"  [ROS] /color_id={color_id} 수신 → place 목표 {goal_pos}")
