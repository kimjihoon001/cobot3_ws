...
class GreenhouseConfig:
    ...
    """
    # 좌우폭(X). 2026-07-19 "세로"×1.5=15 였으나, 2026-07-20 팀 피드백①(X방향)로
    #   섹터를 좌우벽에 붙이고 여백/중앙통로를 로봇폭 기준으로 줄여 딱 맞춤 → 13.0.
    #   = 재배구역 폭(bed edge ±3.46) + 좌우 벽여백 3.0m(=MM 3대) 양쪽. [4] 임의(시연).
    width: float = 13.0
...
class PlantConfig:
    ...
    """
    # 조간 (m). 섹터 내 이랑 사이 = MM 수확 통로. 하이와이어 조간 1.5~1.8m(UMass/TAMU)
    #   상한을 택해 MM(풋프린트 1.05m)이 통로에 여유롭게 들어가게 한다(≥1.5×1.05=1.58m).
    #   [1] 출처 범위 안(1.5~1.8m). 1.5m 였을 땐 베드(0.42m×2) 빼면 실통로 1.08m 로
    #   로봇 폭 0.79m 에 양쪽 14cm 뿐이라 빠듯했음(팀 지적 2026-07-19). 1.8m → 실통로 1.38m.
    row_spacing: float = 1.8
