...
# ── B-3. 10단계 타이밍 ────────────────────────────────────────
#   핵심: dt 가 "클수록" 빠름.  단계당 스텝 수 = 1 / dt,  시간 ≈ (1/dt) x (1/60)초.
#     예) dt=0.0025 → 400 step ≈ 6.7초(느림) /  dt=0.05 → 20 step ≈ 0.33초(빠름)
#   ★ 너무 키우면 실패하는 단계:
#       1·6 (하강)  : 팔이 도달하기 전에 넘어가 큐브를 치거나 헛집음
#       2   (닫기)  : 그리퍼가 다 닫히기 전에 넘어가 놓침
#     → 이 세 개는 조심. 나머지(0·4·5·8·9)는 많이 키워도 대체로 안전.

# [경우 1] 정상 — 안정적 (기본값)
EVENTS_DT_NORMAL = [
    0.008,   # 0. 접근 이동         125 step
    0.005,   # 1. 하강              200 step
    0.02,    # 2. 그리퍼 닫기 대기    50 step
    0.1,     # 3. 그리퍼 닫힘 유지    10 step
    0.0025,  # 4. 들어올리기        400 step
    0.01,    # 5. Place 위치로 이동  100 step
    0.0025,  # 6. 하강              400 step
    1,       # 7. 그리퍼 열기 대기     1 step
    0.008,   # 8. 상승              125 step
    0.08,    # 9. 복귀               13 step
]

# [경우 2] 빠름 — 엄청 빠르게 (★ 표시 단계는 실패 위험이라 덜 키움)
EVENTS_DT_FAST = [
    0.05,    # 0. 접근 이동          20 step
    0.02,    # 1. 하강               50 step  ★정밀
    0.05,    # 2. 그리퍼 닫기 대기    20 step  ★놓침 주의
    0.1,     # 3. 그리퍼 닫힘 유지    10 step
    0.03,    # 4. 들어올리기         33 step
    0.06,    # 5. Place 위치로 이동   17 step
    0.02,    # 6. 하강               50 step  ★정밀
    1,       # 7. 그리퍼 열기 대기     1 step
    0.05,    # 8. 상승               20 step
    0.3,     # 9. 복귀                3 step
]

# ── 사용할 프리셋 선택: 아래 두 줄에서 #만 바꾸면 됨 ──
# EVENTS_DT = EVENTS_DT_FAST
EVENTS_DT = EVENTS_DT_NORMAL

# ── 집고 멈춤 옵션 ──
#   True  : Pick(접근~들어올리기, 0~4단계)까지만 하고 정지 (Place 생략)
#   False : 정상 Pick & Place 전체 수행
PICK_ONLY = True
...
def main():
    ...
    while simulation_app.is_running():
        ...
        if is_playing and not task_done:
            ...
            # (2) 집고 멈춤: 들어올리기(4)까지 끝나면(event>=5) 정지, Place 생략
            if PICK_ONLY and controller.get_current_event() >= 5:
                print("[Pick 완료] 큐브 잡고 정지 (Place 생략)")
                task_done = True
                my_world.pause()
            else:
                # Controller에 목표 전달 → 관절 명령 생성
