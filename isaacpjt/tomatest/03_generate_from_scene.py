...
ARM_LIFT_DEG = 0.0         # [사용자] 팔 상하 스윕 끔 → 배포와 같은 HOME 각도로만 촬영(2026-07-23).
...
# 시작자세(HOME_POSE_DEG)를 기본자세로 박고 정착 (rmp_mm.RmpMMDriver.configure 와 동일 로직).
...
# m0617 팔 교체(UR10e→Doosan): 어깨 리프트 = joint_2 (옛 shoulder_lift_joint 대응).
lift_idx = np.array([dof.index("joint_2")])                   # 카메라 상하 스윕용
lift_home = np.radians(dict(HOME_POSE_DEG)["joint_2"])
