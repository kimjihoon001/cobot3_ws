...
class TomatoAssetConfig:
    ...
    # ── 물리 반경 (2026-07-27) ─────────────────────────────────────────────
    # 이 반경(hq_center 기준) 안의 과실만 dynamic 강체 + 구 콜라이더 + 꽃자루
    # FixedJoint 를 갖는다. 밖은 시각 전용이라 PhysX 솔버에서 통째로 빠진다.
    #
    # 왜 필요한가 — 과실 540개 전부에 dynamic 강체 + FixedJoint 를 걸고
    # sleep_threshold=0(절단=낙하 보장용)까지 줘서, 가만히 매달린 과실 540개를
    # PhysX 가 매 스텝 전부 풀고 있었다. 시뮬이 실시간의 0.19배까지 떨어져
    # 컨트롤러 spawner 가 기동 타임아웃으로 죽고 수확 시퀀스가 깨졌다.
    #
    # 실제로 수확하는 건 수확 정차 위치 주변 몇 개뿐이다. 반경은 팔 도달 범위
    # (약 1m)보다 넉넉히 잡아 로봇이 닿을 수 있는 과실은 전부 포함되게 한다.
    # 0 이하로 두면 종전처럼 전 과실에 물리를 붙인다(되돌리기용).
    physics_radius: float = 3.5
