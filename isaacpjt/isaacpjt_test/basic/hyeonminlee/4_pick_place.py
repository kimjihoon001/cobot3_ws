...
def find_prim_path_by_name(root_path: str, name: str):
    """root_path 하위 트리를 순회하며 이름이 name과 일치하는 첫 prim의 경로를 반환한다."""
...
def initialize_robot(robot, world):
    """로봇과 그리퍼를 초기화하고 관절 위치를 0으로 리셋한다."""
    robot.initialize()
...
class M0609Task(BaseTask):
    ...
    def __init__(self, name):
        """Task 이름을 설정하고 목표 달성 여부 플래그를 초기화한다."""
        super().__init__(name=name, offset=None)
        self._task_achieved = False

    def set_up_scene(self, scene):
        """USD 로드 → 링크 탐색 → 물리 설정 → 로봇 등록 → 씬 구성 순서로 전체 씬을 세팅한다."""
    ...
    def _load_usd(self):
        """USD_PATH의 씬 파일을 /World에 레퍼런스로 추가하고 로드될 때까지 앱을 업데이트한다."""
        print("\n" + "=" * 60)
        print("[1.LOAD] USD 로드")
        print("=" * 60)
        stage = omni.usd.get_context().get_stage()
    ...
    def _discover_links(self):
        """엔드이펙터(EE_LINK_NAME)와 그리퍼 조인트들의 prim 경로를 탐색해 저장/출력한다."""
        print("\n" + "=" * 60)
    ...
    def _setup_physics(self):
        """로봇의 모든 관절 DriveAPI에 stiffness/damping/max force 값을 적용한다."""
        print("\n" + "=" * 60)
    ...
    def _register_robot(self, scene):
        """ParallelGripper를 생성하고 SingleManipulator로 로봇을 씬에 등록한다."""
        print("\n" + "=" * 60)
    ...
    def _create_scene(self, scene):
        """대상 큐브, 목표 마커, 물리 머티리얼(큐브/손가락 마찰)을 씬에 생성/적용한다."""
        print("\n" + "=" * 60)
    ...
    def get_observations(self):
        """로봇의 현재 관절 위치와 큐브의 현재/목표 위치를 딕셔너리로 반환한다."""
    ...
    def pre_step(self, control_index, simulation_time):
        """매 스텝마다 큐브가 목표 위치에 도달했는지 확인하고, 도달 시 큐브 색을 초록색으로 바꾼다."""
        cube_pos, _ = self._cube.get_world_pose()
    ...
    def post_reset(self):
        """리셋 시 그리퍼를 열림 상태로, 큐브 색을 파란색으로, 목표 달성 플래그를 초기화한다."""
...
def main():
    """World/Task/로봇을 초기화하고 PickPlaceController를 생성해 Pick & Place 시뮬레이션 루프를 실행한다."""
