...
class TomatoAssetConfig:
    ...
    usd_dir: str = os.path.join(ISAAC_DIR, "tomatest", "tomato_assets_usd")
...
class PlantConfig:
    ...
    fruit_offset: float = 0.09     # 줄기 중심에서 과실까지 거리

    # aoc 배경 식물(잎+가지 메시)을 원기둥 줄기 위에 얹을지. 시각 전용(콜라이더 없음).
    # 켜도 원기둥 줄기(콜라이더)와 obj 과실(수확대상)은 그대로 유지된다 — 줄기 교체는
    # 별도 판단. 기본 False 로 기존 동작 불변. 에셋 없으면 자동으로 건너뛴다.
    use_aoc_background: bool = False

    # 데모 씬의 익음 분포. 스마트팜은 관리되는 환경이라 old 가 드물다.
    # 주의 1: 이 비율 자체는 출처가 없다. 관측 데이터가 아니라 씬 연출값이다.
    ...
    class_weights: dict[str, float] = field(default_factory=lambda: {
        "green":      0.35,
        "half_ripe":  0.35,
        "fully_ripe": 0.25,   # 수확 대상
        "old":        0.05,   # 제거 대상. 관리된 환경이므로 드묾 (VegNet Old)
    })
...
class PhysicsConfig:
    ...
    #   완숙에 가까울수록 손상 압력이 낮아진다. **수확 대상이 fully_ripe 이므로
