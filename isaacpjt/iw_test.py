...
def _fix_articulation_to_world(stage: Usd.Stage, body_path: str) -> bool:
    """현재 월드 자세 그대로 아티큘레이션을 고정한다.

    매 프레임 set_world_pose를 호출하면 PhysX 적분과 텔레포트가 충돌해 로봇이
    날아갈 수 있다. 월드 FixedJoint는 물리 솔버 안에서 같은 자세를 안정적으로 유지한다.
    """
    ...
    if body is None or not body.IsValid():
        print("[WarehouseTest] AMR 고정용 rigid body를 찾지 못했습니다")
        return False
    ...
    joint = UsdPhysics.FixedJoint.Define(stage, "/World/WarehouseTestIwHubFixed")
    joint.CreateBody1Rel().SetTargets([body_path])
    joint.CreateCollisionEnabledAttr(False)
    # body0을 비우면 월드가 기준 강체다. 월드 쪽 앵커를 현재 body 원점에 둔다.
    joint.CreateLocalPos0Attr(Gf.Vec3f(*[float(v) for v in position]))
...
class IwDriver(Driver):
    ...
    def finalize(self, world, stage, opts):
        ...
        if self._warehouse_test:
            ...
            if _fix_articulation_to_world(stage, self.art):
