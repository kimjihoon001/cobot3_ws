...
또한 상하차가 시작되면 AMR을 기준 도킹 pose로 맞춘 뒤 월드 FixedJoint로
고정한다. 팔레트를 내려놓을 때는 포크 FixedJoint를 해제한 같은 물리 프레임에
IW chassis와 팔레트 사이 Deck FixedJoint를 생성한다. 지게차가 대기 위치까지
빠지면 월드 고정만 해제하므로 IW는 이동할 수 있고, 팔레트의 IW 기준 XYZ/RPY는
그대로 유지된다. IW가 돌아오면 다시 기준 pose에 고정하고 DeckJoint를 포크
FixedJoint로 원자적으로 전환한 뒤 직선 회수한다.
...
| `/forklift_0/joint_command` | `sensor_msgs/JointState` | 출력 | 포크·조향·구동, `pallet_attach`/`pallet_deck_attach` 소유권 및 `iw_dock_lock` 명령 |
