"""수확 미션 FSM: 섹터 이동→근거리 탐색→수확→IW 인계·도킹까지 조율한다.

IW 없이 돌릴 때는 `mock_basket_release_xyz` 계열 파라미터로 모의 바스켓 배치 경로를
그대로 쓸 수 있다 (config/nav_harvest_test.yaml).
"""
...
class HarvestFsmNode(Node):
    """Nav2 도착·비전·매니퓰레이터·IW 미션을 잇는 수확 미션 코디네이터."""

    def __init__(
        self,
        node_name: str = "harvest_fsm_node",
        ...
        # 기본 HarvestFsmNode는 기존 동작을 보존하기 위해 auto_nav_goal=false다.
        self.declare_parameter(
        ...
        self._place_more_pending_pub = self.create_publisher(
            Bool,
            str(self.get_parameter("place_more_pending_topic").value),
            latched)
        ...
        self._placed = False
        self._place_count = 0
        self._publish_place_more_pending()
        ...
        # 단계형 피항 FSM. 각 단계는 (라벨, x, y, yaw)이고, 직전 단계가
        # SUCCEEDED 했을 때만 다음 단계를 보낸다.
        self._yield_stages: list[tuple[str, float, float, float]] = []
    ...
    def _nav_status_callback(self, msg: GoalStatusArray) -> None:
        ...
        for entry in msg.status_list:
            ...
            if entry.status == GoalStatus.STATUS_SUCCEEDED:
                ...
                self._active_goal = None
                if goal_id == self._yield_goal_id:
                    self._yield_stage_failed("NAV_FAILED")
                elif goal_id in self._yield_goal_ids:
                    continue
                else:
                    self._publish_enable(False)
    ...
    def _begin_post_nav_home(self) -> None:
        ...
        if self._home_then_bed_view:
            self._resolve_bed_view_pose("Nav2 도착 확정")
        self._publish_home_command()
        self._publish_status(
            "WAIT_ARM_HOME_BEFORE_BED_VIEW"
            if self._home_then_bed_view else "WAIT_ARM_HOME_AFTER_NAV")

    def _resolve_bed_view_pose(self, reason: str) -> None:
        """가까운 베드를 보는 BED_VIEW joint_1 각도를 결정한다."""
        forced_side = str(
        ...
        self.get_logger().info(
            f"{reason}: 가까운 베드={self._bed_view_side}, "
    ...
    def _depart_with_partial_load(self, reason: str) -> bool:
        ...
        수확이 최종 실패했을 때 데크에 이미 실린 과실을 태운 채 멈춰 있으면
        데모가 그 자리에서 끝난다. 단, 한 개도 못 실었으면 빈 IW를 하역장으로
        보내지 않는다 — 보낼 팔레트가 없다.
        """
        if self._iw_full or self._place_count <= 0:
            return False
        target = self._place_target_count()
        self._iw_full = True
        self._publish_place_more_pending()
        self._iw_yield_complete_pub.publish(Bool(data=False))
        self._iw_mission_pub.publish(String(data="PREPARE_FORKLIFT"))
        self._publish_status("WAITING_IW_YIELD_REQUEST_PARTIAL")
        self.get_logger().warning(
            f"{reason} — 부분 적재 {self._place_count}/{target}로 "
            "IW 하역 준비, MM 피항 요청 대기")
        return True

    def _publish_place_more_pending(self) -> None:
        """지금 진행할 플레이스 '다음에도' 또 놓을 게 남는지를 알린다.

        매니퓰레이터는 플레이스를 끝낸 직후 이 값을 보고 HOME 복귀 여부를
        정한다. 그 시점에는 코디네이터가 아직 카운트를 못 올렸으므로,
        완료 개수가 아니라 '이번 것을 포함한 다음'을 기준으로 발행한다.
        """
        self._place_more_pending_pub.publish(Bool(
            data=self._place_count + 1 < self._place_target_count()))

    def _begin_post_place_bed_view(self) -> None:
        """플레이스 직후 접힌 자세에서 joint_1만 돌려 베드뷰로 복귀한다.

        접힌 자세는 HOME_Q에서 joint_1만 바구니 방위인 상태이고 BED_VIEW도
        HOME_Q에서 joint_1만 베드 방위다. 따라서 HOME을 거치지 않고 바로
        BED_VIEW를 보내면 joint_1 한 축만 도는 최단 복귀가 된다.
        """
        self._publish_enable(False)
        self._search_deadline_ns = 0
        self._post_nav_settle_deadline_ns = 0
        self._waiting_home = True
        self._home_then_bed_view = True
        self._waiting_bed_view = True
        self._home_request_id += 1
        timeout = float(self.get_parameter("home_settle_timeout_sec").value)
        self._home_deadline_ns = (
            self.get_clock().now().nanoseconds + int(timeout * 1e9))
        self._resolve_bed_view_pose("플레이스 후 복귀")
        self._publish_home_command()
        self._publish_status("WAIT_ARM_BED_VIEW_AFTER_PLACE")
    ...
    def _manipulator_state_callback(self, msg: String) -> None:
        ...
        if state in {
            ...
            # place_target_count개를 iw 데크에 놓아야 만재 → iw 를 지게차로.
            if not self._cycle_failed and self._placed and not self._iw_full:
                self._placed = False
                self._place_count += 1
                target = self._place_target_count()
                if self._place_count < target:
                    # 아직 남았다 — 홈 왕복 없이 접힌 자세에서 joint_1만
                    # 베드 방위로 돌려 바로 다음 픽앤플레이스를 시작한다.
                    self._publish_place_more_pending()
                    self.get_logger().info(
                        f"적재 {self._place_count}/{target} — "
                        "베드뷰로 복귀해 한 번 더 수확")
                    self._begin_post_place_bed_view()
                    return
                self._publish_place_more_pending()
                self._iw_full = True
                self._iw_yield_complete_pub.publish(Bool(data=False))
                ...
                self.get_logger().info(
                    f"적재 {self._place_count}/{target}(만재) → "
                    "IW 하역 준비, MM 피항 요청 대기")
            elif self._cycle_failed:
                # 매니퓰레이터가 모든 재시도를 소진하고 홈으로 돌아온 상태다
                # (중간 재시도는 RETRY_VISION으로 빠지므로 여기 오지 않는다).
                # 이미 실은 게 있으면 목표 개수를 못 채웠어도 하역을 시작한다.
                self._depart_with_partial_load("수확 최종 실패")
    ...
    def _iw_yield_request_callback(self, msg: Bool) -> None:
        ...
        if not self._nav_client.server_is_ready():
            ...
            return
        stages = self._build_yield_stages()
        if not stages:
            # _build_yield_stages가 실패 상태를 이미 발행했다. 허가는 주지 않는다.
            self._iw_yield_complete_pub.publish(Bool(data=False))
            return
    ...
    def _iw_yield_goal_response(self, future) -> None:
        ...
        try:
            ...
            self._yield_stage_failed("SEND_FAILED")
            return
        if handle is None or not handle.accepted:
            self._yield_stage_failed("REJECTED")
            return
        goal_id = bytes(handle.goal_id.uuid)
    ...
    def _costmap_cost(self, x: float, y: float) -> int:
        ...
        i = int((x - float(info.origin.position.x)) / res)
        j = int((y - float(info.origin.position.y)) / res)
        if not (0 <= i < info.size_x and 0 <= j < info.size_y):
            return -1
    ...
    def _build_yield_stages(self):
        ...
        try:
            ...
            self.get_logger().error(f"MM 피항 TF 실패: {exc}")
            self._publish_status("ERROR_MM_YIELD_TF")
            return None
        if self._costmap is None:
            self.get_logger().error(
                "MM 피항: global costmap 미수신 — waypoint 검증 불가")
            self._publish_status("ERROR_MM_YIELD_NO_COSTMAP")
            return None
    ...
    def _send_yield_stage(self) -> None:
        ...
        self._publish_enable(False)
        self._publish_status(
            f"MM_YIELD_{self._yield_stage_index + 1}_{label}_SENDING")
        future = self._nav_client.send_goal_async(goal)
        future.add_done_callback(self._iw_yield_goal_response)
        self.get_logger().info(
            f"MM 피항 {self._yield_stage_index + 1}/{len(self._yield_stages)} "
            f"{label}: map=({x:.2f}, {y:.2f}), yaw={math.degrees(yaw):.1f}°")
    ...
    def _yield_stage_failed(self, reason: str) -> None:
        index = self._yield_stage_index
    ...
    def _watchdog(self) -> None:
        ...
        if (self._search_deadline_ns
            ...
            # 더 딸 게 없어 탐색이 만료된 경우에도 이미 실은 건 내보낸다.
            self._depart_with_partial_load("토마토 탐색 타임아웃")
